...
download_and_compile_whisper() {
    start_step "Whisper.cpp Download and Compilation"
    
    # Activate virtual environment
    source "${VENV_DIR}/bin/activate"
    
    # Run setup script with enhanced error handling
    log_info "Running whisper setup..."
    python3 setup.py base || error_exit "Failed to setup whisper.cpp"
    
    # Verify whisper executable
    WHISPER_PATHS=(
        "${SCRIPT_DIR}/whisper.cpp/main"
    ...
    for path in "${WHISPER_PATHS[@]}"; do
        if [[ -x "${path}" ]]; then
            log_success "Whisper executable found: ${path}"
            WHISPER_FOUND=true
            break
        fi
    done
    ...
    if [[ "${WHISPER_FOUND}" == "false" ]]; then
        error_exit "Whisper executable not found after compilation"
    fi
...
create_configuration() {
    ...
    if [[ ! -f "${ENV_FILE}" ]]; then
        ...
        cat > "${ENV_FILE}" << EOF
# API Configuration
API_ENDPOINT=https://your-api.com/transcription
...
ExecStart=${VENV_DIR}/bin/python3 mainWithServer.py
Restart=always
RestartSec=10
...
exec python3 mainWithServer.py
EOF
